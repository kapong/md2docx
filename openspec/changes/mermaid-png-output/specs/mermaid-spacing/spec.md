...
The system SHALL add configurable vertical spacing before and after mermaid diagram image paragraphs in the DOCX output. The spacing SHALL be applied as OOXML paragraph spacing properties (`w:spacing w:before` / `w:after`). This applies regardless of whether the output format is PNG or SVG.

#### Scenario: Default spacing applied

- **WHEN** a mermaid diagram is rendered (PNG or SVG) and no `[mermaid]` config section exists
...
The spacing SHALL be applied to the paragraph containing the mermaid image element (PNG or SVG). It SHALL NOT affect the caption paragraph spacing (which is controlled by the image template caption settings).
...
- `output_format`: image format `"png"` (default) or `"svg"`
...
- **THEN** the system SHALL parse `spacing_before`, `spacing_after`, `output_format`, and `dpi` fields
