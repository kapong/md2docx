...
| `dump-template` | Generate a template file / สร้างไฟล์แม่แบบ |
| `validate-template` | Check template validity / ตรวจสอบความถูกต้องของแม่แบบ |
...
## dump-template Command {#ch07-dump-template}

### English

The `dump-template` command generates a starter DOCX template with all required styles. This is the recommended starting point for creating custom templates.

### ภาษาไทย

คำสั่ง `dump-template` สร้างแม่แบบ DOCX เริ่มต้นที่มีสไตล์ที่จำเป็นทั้งหมด นี่เป็นจุดเริ่มต้นที่แนะนำสำหรับการสร้างแม่แบบกำหนดเอง

### Usage / การใช้งาน

```bash
md2docx dump-template [OPTIONS]
```
...
| Option | Short | Type | Default | Description |
|--------|-------|------|---------|-------------|
| `--output` | `-o` | string | required | Output filename / ชื่อไฟล์เอาต์พุต |
| `--lang` | | string | `"en"` | Language preset (`en` or `th`) / ค่าที่ตั้งล่วงหน้าตามภาษา |
| `--minimal` | | boolean | `false` | Generate minimal template / สร้างแม่แบบขั้นต่ำ |
...
- Code font: Consolas
- Colors: Blue headings

**Thai (`--lang th`):**
- Default font: TH Sarabun New (14pt)
- Heading font: TH Sarabun New
- Code font: Consolas
- Colors: Blue headings
- Larger base size for Thai readability

### dump-template Examples {#ch07-dump-template-examples}

```bash
# Generate default English template
...
md2docx dump-template -o ./templates/company.docx
```

---

## validate-template Command {#ch07-validate-template}

### English

The `validate-template` command checks if a DOCX file contains all required styles for md2docx. Use this to verify templates before use.

### ภาษาไทย

คำสั่ง `validate-template` ตรวจสอบว่าไฟล์ DOCX มีสไตล์ที่จำเป็นทั้งหมดสำหรับ md2docx หรือไม่ ใช้เพื่อตรวจสอบแม่แบบก่อนใช้งาน

### Usage / การใช้งาน

```bash
md2docx validate-template <TEMPLATE_FILE> [OPTIONS]
```
...
|----------|----------|-------------|
| `TEMPLATE_FILE` | Yes | Path to template DOCX file / พาธไปยังไฟล์แม่แบบ DOCX |

### Options / ตัวเลือก

| Option | Short | Type | Description |
|--------|-------|------|-------------|
| `--verbose` | `-v` | boolean | Show detailed output / แสดงเอาต์พุตโดยละเอียด |

### Exit Codes / รหัสออก

| Code | Meaning |
|------|---------|
| `0` | Template is valid / แม่แบบถูกต้อง |
| `1` | Template is invalid / แม่แบบไม่ถูกต้อง |
| `2` | File not found / ไม่พบไฟล์ |

### validate-template Examples {#ch07-validate-template-examples}
...
if [ $? -eq 0 ]; then
    ...
    echo "Template has issues"
fi
```

### Sample Output / ตัวอย่างเอาต์พุต

**Valid Template / แม่แบบถูกต้อง:**
...
Missing required styles (2):
  ✗ Code
  ✗ CodeChar

Missing recommended styles (1):
  ⚠ Caption

To fix, run: md2docx dump-template -o template.docx
```
