...
fn extract_caption_runs(p_xml: &str) -> Vec<CaptionRun> {
    ...
    while let Some(r_start) = p_xml[pos..].find("<w:r") {
        ...
        if let Some(r_end) = p_xml[abs_r_start..].find("</w:r>") {
            ...
            let (font, size, color, bold, italic) = extract_run_properties(r_xml);
...
fn extract_run_properties(xml: &str) -> (String, u32, String, bool, bool) {
    ...
    if let Some(rpr_start) = xml.find("<w:rPr") {
        if let Some(rpr_end) = xml[rpr_start..].find("</w:rPr>") {
            ...
            if rpr_xml.contains("<w:b/>")
                || rpr_xml.contains("<w:b ")
                || rpr_xml.contains("<w:bCs/>")
            {
                bold = true;
            }
            if rpr_xml.contains("<w:i/>")
                || rpr_xml.contains("<w:i ")
                || rpr_xml.contains("<w:iCs/>")
            {
                italic = true;
            }
...
mod tests {
    ...
    fn test_debug_extract_real_template() {
        let path = Path::new(
            "/Users/kapong/workspace/lab/md2docx/examples/thai-manual/template/image.docx",
        );
        if !path.exists() {
